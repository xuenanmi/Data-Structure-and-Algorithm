...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.find(word1_) == dict.end()) return false;
    if (dict.find(word2_) == dict.end()) return false;

    if (dict.at(word1_).size() != dict.at(word2_).size()) return false;

    for (size_t i = 0; i < dict.at(word1_).size(); i++){
       if (dict.at(word1_).at(i) != dict.at(word2_).at(i)){
          return false;
       }
