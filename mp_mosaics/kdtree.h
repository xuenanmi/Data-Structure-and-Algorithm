...
class KDTree
{
  ...
  private:
   ...
   int partition(int curDim, int left, int right, int pivotIndex);
   ...
   Point<Dim> findNearestNeighbor(Point<Dim> query, Point<Dim> curBest, int dim, KDTreeNode* curRoot) const;
};
