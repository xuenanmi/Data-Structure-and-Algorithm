...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      // double distance = sqrt(pow(fabs(centerX - x),2.0) + pow(fabs(centerY - y),2.0));
      if (distance < 160) {pixel.l = (1 - distance * 0.005) * (pixel.l);}
      else {pixel.l = (1 - 160 * 0.005) * (pixel.l);}
      }
    }                         
  return image;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned int x = 0; x < firstImage.width(); x++){
    for (unsigned int y = 0; y < firstImage.height(); y++){
      ...
      if (pixel2.l == 1) {
        if (pixel1.l <= 0.8) {pixel1.l = pixel1.l + 0.2;}
        else if (pixel1.l > 0.8) {pixel1.l = 1.0;}
      }
    }
  } 
  return firstImage;
