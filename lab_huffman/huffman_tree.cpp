...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     //second step;
     TreeNode* internal_left = nullptr;
     TreeNode* internal_right = nullptr;
     TreeNode* internal = nullptr;   
 
     while (singleQueue.size() + mergeQueue.size() != 1){
        internal_left = removeSmallest(singleQueue, mergeQueue);       
        internal_right = removeSmallest(singleQueue, mergeQueue);
        internal = new TreeNode(internal_left->freq.getFrequency() + internal_right->freq.getFrequency());
        internal->left = internal_left;
        internal->right = internal_right;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit()) {
      leaf = new TreeNode(Frequency(bfile.getNextByte(),0));
      return leaf;
    }
